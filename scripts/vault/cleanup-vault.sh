...
# Удаляем секреты Redis/Celery
echo "Удаляем секреты Redis/Celery из ${PROJECT_NAME}/dev/config..."
if vault kv metadata delete secret/${PROJECT_NAME}/dev/config; then
  success_print "Секреты Redis/Celery успешно удалены"
else
  echo "Предупреждение: Не удалось удалить секреты Redis/Celery. Возможно, они уже были удалены."
fi
...
echo "5. Секреты Redis/Celery по пути secret/${PROJECT_NAME}/dev/config"
echo "======================================================"