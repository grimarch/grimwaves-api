...
# Grant read access to all dev secrets for the project
path "secret/data/${PROJECT_NAME}/dev/*" {
  capabilities = ["read"]
}
EOF
...
# Добавление секретов Redis/Celery
echo ""
echo "=== Настройка Redis и Celery URL для проекта ${PROJECT_NAME} ==="
echo "Сохранение Redis и Celery URL в Vault..."
if vault kv put secret/${PROJECT_NAME}/dev/config \
       CELERY_BROKER_URL="redis://redis:6379/0" \
       CELERY_RESULT_BACKEND="redis://redis:6379/0" \
       REDIS_URL="redis://redis:6379/1"; then
    success_print "URL Redis и Celery успешно сохранены в Vault по пути secret/${PROJECT_NAME}/dev/config"
else
    error_exit "Не удалось сохранить URL Redis и Celery в Vault"
fi

# Проверка, что секреты Redis/Celery действительно записаны
echo "Проверка записи секретов Redis/Celery..."
if ! vault kv get -format=json secret/${PROJECT_NAME}/dev/config > /dev/null; then
    error_exit "Не удалось проверить наличие секретов Redis/Celery в Vault."
fi
success_print "Секреты Redis/Celery успешно проверены"
...
echo "4. URL Redis/Celery по пути secret/${PROJECT_NAME}/dev/config"
echo ""
echo "Учетные данные сохранены:"
echo "Role ID записан в: ${PROJECT_DIR}/vault-agent/auth/role-id"

if [ "${USE_WRAPPED_TOKEN:-false}" == "true" ]; then
  echo "Wrapped Token (для Secret ID) записан в: ${PROJECT_DIR}/vault-agent/auth/wrapped-secret-id"
  echo ""
  ...
  echo "Secret ID записан в: ${PROJECT_DIR}/vault-agent/auth/secret-id"
  echo ""
